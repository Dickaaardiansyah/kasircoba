// src/views/pages/CashRegister.jsx
// ─────────────────────────────────────────────────────────────────────────────
...
import { useCashRegisterPresenter } from "../../presenters/useCashRegisterPresenter";
...
export default function CashRegister() {
  const cr = useCashRegisterPresenter();
  const { isAdmin } = useAuth();
  // Riwayat lintas-shift (semua kasir) khusus admin — kasir hanya melihat &
  // mengelola sesi kas miliknya sendiri yang sedang berjalan ("Biaya").
  const tabs = isAdmin ? TABS : TABS.filter((t) => t.id !== "riwayat");
  ...
  return (
    <div className="fade-in">
      ...
      <div className="page-body">
        ...
        {cr.tab === "kas" && (cr.loading ? <PageLoader /> : <KasBerjalan cr={cr} />)}
...
function KasBerjalan({ cr }) {
  ...
  if (!cr.shift) {
    return (
      <>
        <NoShiftScreen onStart={() => setShowOpen(true)} />
        {showOpen && (
          <OpenShiftModal
            opening={cr.opening}
    ...
    <div>
      <div className="mutation-summary mb-4">
        ...
        <div className="mutation-summary__card">
          ...
          <div className="mutation-summary__sub">Modal awal + penjualan tunai + kas masuk − kas keluar</div>
        </div>
      </div>
      ...
      <div className="modal modal--small">
        ...
        <div className="modal-body">
          ...
          <div className="form-group">
            ...
            <input type="number" className="form-input" placeholder="Contoh: 10000" value={amount} onChange={(e) => setAmount(e.target.value)} />
          </div>
          <div className="form-group">
