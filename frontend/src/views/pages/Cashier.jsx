// src/views/pages/Cashier.jsx
import { useState } from "react";
import { Search, ScanBarcode, Trash2, Plus, Minus, ShoppingCart, Printer, X, CheckCircle2 } from "lucide-react";
import { useCashierPresenter, PAYMENT_METHODS } from "../../presenters/useCashierPresenter";
...
export default function Cashier() {
  ...
  if (!shift) {
    return (
      <>
        <NoShiftScreen onStart={() => setShowOpenShift(true)} />
        {showOpenShift && (
          ...
          <div className="product-grid">
            {p.filteredProducts.map((product) => (
              ...
              >
                ...
                <div className="product-stock">Stok: {product.stock} {product.unit}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="pos-cart">
            ...
            {p.cart.map((item) => (
              <div key={item.id} className="cart-item">
                <div style={{ flex: 1 }}>
                  ...
                  <div className="cart-item-price">{formatRupiah(item.price)} x {item.qty}</div>
                  {Number(item.price_wholesale) > 0 && (
                    item.priceType === "wholesale" ? (
                      ...
                      <div className="cart-item-wholesale-badge">
                        Grosir otomatis mulai {item.min_qty_wholesale} {item.unit}
                      </div>
                    )
                  )}
                </div>
                <button className="cart-qty-btn" onClick={() => p.changeQty(item.id, -1)}><Minus size={12} /></button>
                <span className="cart-qty">{item.qty}</span>
                <button className="cart-qty-btn" onClick={() => p.changeQty(item.id, 1)}><Plus size={12} /></button>
                <button className="btn btn-ghost btn-icon btn-sm" onClick={() => p.removeFromCart(item.id)}><X size={14} /></button>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{ padding: 16, borderTop: "1px solid var(--border)" }}>
          <div className="form-group">
            ...
            <input
              type="number" className="form-input" value={p.discount || ""}
              onChange={(e) => p.setDiscount(parseFloat(e.target.value) || 0)} placeholder="0"
            />
          </div>
          ...
          <button className="btn btn-primary btn-lg w-full mt-3" onClick={p.openPaymentModal} disabled={p.cart.length === 0}>
            Bayar Sekarang
          </button>
        </div>
      </div>
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <input
                  type="number" className="form-input" value={p.paymentAmount}
                  onChange={(e) => p.setPaymentAmount(e.target.value)}
                />
              </div>
