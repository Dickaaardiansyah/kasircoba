// src/views/pages/StockOpname.jsx
// ─────────────────────────────────────────────────────────────────────────────
...
import { useStockOpnamePresenter, useStockOpnameFormPresenter } from "../../presenters/useStockOpnamePresenter";
...
export default function StockOpname() {
  const so = useStockOpnamePresenter();
              ...
              <tbody>
                {session.items?.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="font-mono">{item.system_stock} {item.unit}</td>
                    <td className="font-mono">{item.physical_stock} {item.unit}</td>
...
function NewOpnameForm({ onSuccess }) {
  const f = useStockOpnameFormPresenter(onSuccess);
            ...
            {visibleProducts.length === 0 ? (
              ...
              <div key={p.id} className="cart-item" style={{ cursor: "pointer" }} onClick={() => f.addItem(p)}>
                <div style={{ flex: 1 }}>
                  ...
                  <div className="cart-item-price">Stok Sistem: {p.stock} {p.unit} • {p.barcode}</div>
                </div>
                <Plus size={16} />
              </div>
            ))}
