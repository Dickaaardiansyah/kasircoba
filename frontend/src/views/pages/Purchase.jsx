// src/views/pages/Purchase.jsx
import { useState } from "react";
import { Plus, Trash2, Eye, X, Truck, Paperclip, FileText } from "lucide-react";
import { usePurchasePresenter, usePurchaseFormPresenter } from "../../presenters/usePurchasePresenter";
...
function NewPurchaseForm({ products, suppliers, onSuccess }) {
  const f = usePurchaseFormPresenter(products, onSuccess);
          ...
          {filteredProducts.map((p) => (
            <div key={p.id} className="cart-item" style={{ cursor: "pointer" }} onClick={() => f.addItem(p)}>
              <div style={{ flex: 1 }}>
                ...
                <div className="cart-item-price">Modal: {formatRupiah(p.cost_price)} • Stok: {p.stock} {p.unit}</div>
              </div>
              <Plus size={16} />
            </div>
          ))}
              ...
              <div style={{ flex: 1, width: "100%" }}>
                ...
                <div className="purchase-item-fields">
                  <div className="purchase-item-field">
                    <label>Qty</label>
                    <input type="number" className="form-input" value={item.quantity} onChange={(e) => f.updateItem(item.product_id, "quantity", e.target.value)} />
                  </div>
                  <div className="purchase-item-field">
                    <label>Harga Modal</label>
                    <input type="number" className="form-input" value={item.unit_cost} onChange={(e) => f.updateItem(item.product_id, "unit_cost", e.target.value)} />
                  </div>
                  <div className="purchase-item-field">
