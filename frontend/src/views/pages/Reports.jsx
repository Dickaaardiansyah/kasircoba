...
import {
  useReportsPresenter, SALES_SORT_OPTIONS, PURCHASE_SORT_OPTIONS,
  QUICK_RANGE_OPTIONS, PROFIT_SORT_OPTIONS, CUSTOMER_SORT_OPTIONS,
} from "../../presenters/useReportsPresenter";
import { PageLoader, StatCard, SectionHeader, EmptyState, Badge } from "../components/UI";
import { formatRupiah, formatDate, formatChartPeriod, formatNumber } from "../../utils/format";
...
function buildExportPayload(r) {
  if (r.reportType === "penjualan" && r.salesReport) {
    ...
    return {
      ...
      columns: [
        { key: "name", label: "Produk" }, { key: "total_qty", label: "Qty" }, { key: "total_revenue", label: "Pendapatan" },
      ],
      rows: (r.sortedSalesTopProducts || []).map((p) => ({ name: p.name, total_qty: p.total_qty, total_revenue: formatRupiah(p.total_revenue) })),
      summary: [
  ...
  if (r.reportType === "penjualanPelanggan" && r.customerReport) {
    ...
    return {
      ...
      rows: (r.sortedCustomers || []).map((c) => ({
        customer_name: c.customer_name, transaction_count: c.transaction_count, total_qty: c.total_qty,
  ...
  if (r.reportType === "labaProduk" && r.profitReport) {
    ...
    return {
      ...
      columns: [
        { key: "name", label: "Produk" }, { key: "total_qty", label: "Qty" },
  ...
  if (r.reportType === "barangMasuk" && r.purchaseReport) {
    ...
    return {
      ...
      rows: (r.sortedPurchaseTopProducts || []).map((p) => ({ product_name: p.product_name, total_qty: p.total_qty, total_cost: formatRupiah(p.total_cost) })),
      summary: [
        { label: "Total Pembelian", value: s.total_purchases || 0 },
        { label: "Total Qty Masuk", value: s.total_qty || 0 },
  ...
  if (r.reportType === "barangExpired" && r.expiredReport) {
    ...
    return {
      ...
      summary: [
        ...
        { label: "Qty Kadaluarsa", value: s.total_qty_expired || 0 },
      ],
    };
  }
  return {};
            ...
            <table>
              ...
              <tbody>
                {r.sortedSalesTopProducts.map((p, i) => (
                  <tr key={i}><td>{p.name}</td><td>{p.total_qty}</td><td className="font-mono">{formatRupiah(p.total_revenue)}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
            ...
            <table>
              ...
              <tbody>
                {rep.categoryRevenue.map((c, i) => (
                  <tr key={i}><td>{c.category}</td><td>{c.qty}</td><td className="font-mono">{formatRupiah(c.revenue)}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ...
        {items.length === 0 ? (
          ...
          <div className="table-container">
            <table>
              <thead>
                <tr>
                  <th>Produk</th><th>Kategori</th><th>Qty Terjual</th>
  ...
  return (
    <>
      <div className="stats-grid">
        ...
        <StatCard icon={Truck} tone="green" label="Total Qty Masuk" value={s.total_qty || 0} />
            ...
            <table>
              ...
              <tbody>
                {r.sortedPurchaseTopProducts.map((p, i) => (
                  <tr key={i}><td>{p.product_name}</td><td>{p.total_qty}</td><td className="font-mono">{formatRupiah(p.total_cost)}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
            ...
            <table>
              ...
              <tbody>
                {(rep.perSupplier || []).map((c, i) => (
                  <tr key={i}><td>{c.supplier}</td><td>{c.total_qty}</td><td className="font-mono">{formatRupiah(c.total_cost)}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
  ...
  return (
    <>
      <div className="stats-grid">
        ...
        <StatCard icon={CheckCircle2} tone="green" label="Qty Kadaluarsa" value={s.total_qty_expired || 0} />
      </div>
                ...
                {items.map((it) => {
                  ...
                  return (
                    <tr key={it.id}>
                      ...
                      <td>{it.current_stock ?? "-"} {it.unit}</td>
