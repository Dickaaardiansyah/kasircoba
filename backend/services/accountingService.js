...
const accountingService = {
  ...
  async createExpense(payload) {
    ...
    const result = await accountingModel.createExpense({
    ...
    const expense = await accountingModel.findExpenseById(result.insertId);

    // Posting jurnal otomatis (Dr Beban sesuai kategori, Cr Kas). Best-effort
    // — lihat catatan desain di services/journalService.js.
    try {
      await journalService.postExpenseJournal(expense);
    } catch (journalError) {
      console.error(
        "Gagal posting jurnal biaya operasional:",
