...
const cashRegisterService = {
  ...
  async createMovement(payload) {
    ...
    // Posting jurnal otomatis (Dr/Cr Kas vs Beban/Modal/Pendapatan Lain-lain
    // tergantung kategori). Best-effort — lihat catatan desain di
    // services/journalService.js.
    try {
      const movements = await cashRegisterModel.findMovementsByShift(shift.id);
      const latest = movements[0];
      if (latest)
        await journalService.postCashMovementJournal(latest, shift.shift_code);
    } catch (journalError) {
      console.error(
        "Gagal posting jurnal pergerakan kas:",
  ...
  async closeShift(id, payload) {
    ...
    await cashRegisterModel.closeShift(id, {
    ...
    const closed = await cashRegisterModel.findShiftById(id);

    // Posting jurnal otomatis untuk selisih kas (jika ada). Best-effort —
    // lihat catatan desain di services/journalService.js.
    try {
      await journalService.postCashShiftCloseJournal(closed);
    } catch (journalError) {
      console.error(
        "Gagal posting jurnal selisih tutup kas:",
        journalError.message,
      );
    }
