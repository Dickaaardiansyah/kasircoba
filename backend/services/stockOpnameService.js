...
const stockOpnameService = {
  ...
  async createSession(payload) {
    ...
    // Posting jurnal otomatis untuk selisih nilai stok (jika ada). Best-effort
    // — lihat catatan desain di services/journalService.js.
    try {
      await journalService.postStockOpnameJournal(session);
    } catch (journalError) {
      console.error("Gagal posting jurnal stock opname:", journalError.message);
    }
