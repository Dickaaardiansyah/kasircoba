...
const capitalService = {
  async record(payload) {
    ...
    const result = await capitalModel.create({
    ...
    const tx = await capitalModel.findById(result.insertId);

    // Posting jurnal otomatis — best-effort, konsisten dengan pola modul lain
    // (lihat catatan desain di services/journalService.js).
    try {
      await journalService.postCapitalJournal(tx);
    } catch (journalError) {
      console.error("Gagal posting jurnal modal usaha:", journalError.message);
    }
