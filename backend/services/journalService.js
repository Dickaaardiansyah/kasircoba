...
//
// Keputusan desain penting: posting jurnal dipanggil SETELAH transaksi utama
// berhasil (bukan dalam satu DB transaction yang sama), dan dibungkus
// try/catch di sisi pemanggil. Jika posting jurnal gagal, operasi POS utama
// (mis. checkout kasir) TETAP berhasil — hanya dicatat sebagai warning di
// log server. Ini sengaja: kasir tidak boleh terhambat karena bug di modul
// akuntansi. Trade-off-nya, jika ada kegagalan, jurnal untuk transaksi itu
// perlu diinput manual lewat menu "Jurnal Manual".
// ─────────────────────────────────────────────────────────────────────────────
...
const journalService = {
  ...
  // Dipanggil dari service modul terkait. Semua fungsi ini best-effort: jika
  // gagal, lempar error biasa — PEMANGGIL yang bertanggung jawab membungkus
  // dengan try/catch supaya operasi utama tidak ikut gagal (lihat komentar
  ...
  // Dipanggil dari receivableService.recordPayment() setelah pembayaran
  // tersimpan (best-effort, sama seperti postSaleJournal).
  async postReceivablePaymentJournal(payment, receivable) {
  ...
  async postPurchaseJournal(purchase) {
  ...
  async postPayablePaymentJournal(payment, payable) {
    const kasCode = payment.payment_method === "cash" ? ACC.KAS : ACC.BANK;
    const amount = round2(Number(payment.amount));
    if (amount <= 0) return null;
    const lines = [
  ...
  async postExpenseJournal(expense) {
  ...
  async postCashMovementJournal(movement, shiftCode) {
  ...
  async postCashShiftCloseJournal(shift) {
  ...
  async postStockOpnameJournal(session) {
